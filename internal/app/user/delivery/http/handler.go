...
func (handler *UserHandler) Login(ctx echo.Context) error {
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx), trace+".Login")
	...
	if err := ctx.Bind(&newUserDataForInput); err != nil {
		...
		logger.CustomLogger.LogErrorInfo(requestId.GetRequestIdFromContext(ctx),
			err.Error())
	...
	if id == errors.NO_USER_ERROR {
		...
		logger.CustomLogger.LogTraceDebug(requestId.GetRequestIdFromContext(ctx),
			trace+" No user: "+newUserDataForInput.Name)
	...
	if id == errors.WRONG_PASSWORD_ERROR {
		...
		logger.CustomLogger.LogTraceDebug(requestId.GetRequestIdFromContext(ctx),
			trace+" Wrong password for user: "+newUserDataForInput.Name)
		return ctx.JSON(http.StatusUnauthorized, newLoginError)
	}
	...
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx),
		trace+"ok login for user: "+newUserDataForInput.Name)
	return ctx.JSON(http.StatusOK, newUserDataForInput)
}

func (handler *UserHandler) SignUp(ctx echo.Context) error {
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx), trace+".SignUp")
	...
	if newId == errors.USER_EXISTS_ERROR {
		...
		logger.CustomLogger.LogTraceDebug(requestId.GetRequestIdFromContext(ctx),
			trace+" User Already exist: "+newUser.Name)
	...
	newUser.Password = ""
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx),
		trace+"ok signup for user: "+newUser.Name)

	return ctx.JSON(http.StatusOK, newUser)
}

func (handler *UserHandler) Profile(ctx echo.Context) error {
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx), trace+".Profile")
	...
	if err != nil {
		logger.CustomLogger.LogErrorInfo(requestId.GetRequestIdFromContext(ctx),
			trace+" "+errors.TOKEN_ERROR_DESCR)
...
func (handler *UserHandler) Logout(ctx echo.Context) error {
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx), trace+".Logout")
...
func (handler *UserHandler) setCookieValue(ctx echo.Context, value string) {
	logger.CustomLogger.LogTrace(requestId.GetRequestIdFromContext(ctx), trace+".setCookieValue")
